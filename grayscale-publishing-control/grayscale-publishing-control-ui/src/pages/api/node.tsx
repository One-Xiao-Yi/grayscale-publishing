...
import ChangeNodeType from "@/pages/api/components/changeNodeType";
import {getResource} from "@/services/com.xiao.yi/api";

const ResourceNode: React.FC = () => {

  const [nodeList, setNodeList] = useState<{ title: string, subTitle: any, actions: any, content: any, node: API.ApiResourceNode }[]>([])
  const [resource, setResource] = useState<API.ApiResource>({nodes: [], resourceName: "", resourceType: undefined})
  const location = useLocation()
  const [actionType, setActionType] = useState<number>(-1)
  const [selectNode, setSelectNode] = useState<API.ApiResourceNode>({});
  const [nodeTypeOpen, setNodeTypeOpen] = useState(false)


  const changeNodeType = (node: API.ApiResourceNode) => {
    setSelectNode(node)
    setActionType(1)
    setNodeTypeOpen(true)
  }

  const closeNode = (node: API.ApiResourceNode) => {
    setSelectNode(node)
  }
  ...
  const getResourceInfo = (resource: API.ApiResource) => {
  ...
  useEffect(() => {
    const resource = location.state as API.ApiResource
